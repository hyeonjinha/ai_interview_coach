...
export default function NewExperiencePage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      const errorMessage = error.response?.data?.detail || '경험 추가에 실패했습니다.';
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-text-primary">새 경험 추가</h1>
          <p className="text-text-secondary">면접에서 활용할 경험을 추가하세요</p>
        </div>
      </div>
        ...
        {selectedCategory && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>경험의 기본적인 정보를 입력해주세요</CardDescription>
            </CardHeader>
        ...
        {selectedCategory && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {getCategoryText(selectedCategory)} 경험에 대한 구체적인 내용을 작성해주세요
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
